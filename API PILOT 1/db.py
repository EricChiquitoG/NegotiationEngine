...
rooms_collection = negengine_db.get_collection("rooms")
...
room_details=negengine_db.get_collection("room_details")
nego=negengine_db.get_collection("negotiations")
nego_details=negengine_db.get_collection("negotiation_details")
user_locations=negengine_db.get_collection("locations")
...
def new_location(user,neg_id,loc_type,name,location):
    user_locations.insert_one(
        {'type':loc_type, #Type of location (main or external warehouse)
        '_id':ObjectId(),
        'user':user,
        'neg_id':neg_id,
        'loc_name':name, #main if main locaiton or else warehouse name 
        'location':location
        })
...
def save_message(room_id, text, sender, sign,distance):
    bids_collection.insert_one({'type':'bid','_id':ObjectId(), 'room_id': room_id, 
